...
const DEFAULTS = {
  ...
  retryableStatuses: [500, 502, 503, 504, 524],
  retryableMessagePatterns: ["upstream", "524", "temporarily unavailable", "rate limit exceeded"],
  maxRetries: 8,
...
export const Config = z.object({
  ...
  retryableStatuses: z.array(z.number()).default([500, 502, 503, 504, 524]),
  retryableMessagePatterns: z.array(z.string()).default(["upstream", "524", "temporarily unavailable", "rate limit exceeded"]),
...
export function apply(ctx, config) {
  ...
  const clearAttemptsFor = (sessionId) => {
  ...
  ctx.on("agent/request-error", async (payload, next) => {
    ...
    try {
      ...
      if (prior >= c.maxRetries) {
        ...
        log.warn?.(
          `[${agent.id}] ${provider} step ${turn}/${step} 重试耗尽（${prior + 1} 次），` +
        ...
        // 这是对「看门狗耗尽 → 放行 → llm-retry 重试 → 计数清零 → 再耗尽」无限乒乓循环的根治。
  ...
  // ---------------- 第二层：回合级复活 ----------------
  function scheduleRestart(agent, failure, count, instructionOverride, isCutoff) {
  ...
  const status = () => {
    ...
    return {
      ...
      stopAfterExhaustion: !!c.stopAfterExhaustion,
  ...
  log.info?.("dsh-relay-watchdog 已启动", {
    ...
    stopAfterExhaustion: cfg().stopAfterExhaustion,
    restartOnTurnError: cfg().restartOnTurnError,
    reviveOnMaxTokens: cfg().reviveOnMaxTokens,
    resetBudgetOnSuccess: cfg().resetBudgetOnSuccess,
